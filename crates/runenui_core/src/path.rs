...
pub(crate) fn quadratic_roots(a: f64, b: f64, c: f64, roots: &mut [f64; 2]) -> usize {
    if a == 0.0 {
        if b == 0.0 {
            return 0;
        }
...
pub(crate) fn quadratic(p0: f64, p1: f64, p2: f64, t: f64) -> f64 {
    let a = (-2.0_f64).mul_add(p1, p0) + p2;
    let b = 2.0 * (p1 - p0);
    a.mul_add(t, b).mul_add(t, p0)
}

pub(crate) fn cubic_coefficients(p0: f64, p1: f64, p2: f64, p3: f64) -> [f64; 4] {
...
pub(crate) fn cubic(p0: f64, p1: f64, p2: f64, p3: f64, parameter: f64) -> f64 {
