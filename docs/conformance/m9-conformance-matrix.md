...
geometry under visual motion, ambient reduced-motion policy, custom backend shader
escape hatch, software expected renderer, or compatibility shim preserving replaced
pre-1.0 visual authority cannot satisfy M9.
...
|---|---|---|---|---|---|---|---|
| M9VIS-01 | Public visual/composition vocabulary is RunenUI-owned, host/renderer-neutral, and contains no Lyon, Kurbo, Peniko, Vello, wgpu, WGSL, native, mounted, or dependency scene types. | Public-API/dependency corpus | Forbidden-type/public-dependency and second-scene-authority audit | Visual schema/ownership report | M9A | blocked | Required |
| M9VIS-02 | Validated rect/rounded-rect/ellipse/path geometry has finite logical coordinates, explicit contour/fill-rule semantics, deterministic logical bounds, and path hit behavior independent of renderer tessellation. | Shape/path/bounds/hit corpus | Non-finite/malformed path and raster/tessellation-derived authority audit | Path validation/bounds/hit records | M9A | blocked | Required |
...
| M9VIS-04 | Solid, linear-gradient, and radial-gradient brushes preserve validated logical geometry, stable nondecreasing stops, hard stops, straight-alpha sRGB8 inputs, and premultiplied linear-sRGB interpolation without backend-specific color policy. | Brush/gradient/color corpus | Unsorted/non-finite stop, hidden color-space, and backend-brush audit | Gradient normalization/interpolation records | M9A | blocked | Required |
...
| M9VIS-07 | Immutable snapshot-local composition groups preserve nesting, clips, source-over/group opacity, and exact ordered membership without acquiring mounted identity, lifecycle, reconciliation, semantic identity, or cross-publication authority. | Group composition corpus | Per-child-opacity substitution and retained-second-tree audit | Group structure/composition records | M9A | blocked | Required |
| M9VIS-08 | Ordinary drop shadows preserve exact neutral offset/sigma/spread/color and deterministic logical effect bounds; shadows expand paint/damage only and do not silently alter layout/hit/focus/semantic bounds. | Shadow/effect-bounds corpus | Backend-default blur, clipped-shadow, and semantic/layout inflation audit | Effect-bound/realization diagnostics | M9A | blocked | Required |
...
| M9MOTION-03 | Explicit timelines are declarative authored values with stable animation identity, ordered normalized keyframes, duration/delay/repeat policy, and exact mounted-generation lifecycle; unchanged identity/spec preserves progress while replacement/removal/unmount/shutdown cancel deterministically. | Reconciliation/timeline-lifetime corpus | Widget-local hidden timer/state-machine and stale-generation corpus | Start/replace/cancel/lifetime records | M9B | blocked | Required |
| M9MOTION-04 | Linear/cubic-Bezier easing and timeline normalization have exact endpoints, deterministic delay/zero-duration/iteration/completion semantics, finite checked time arithmetic, and forever timelines never claim natural completion. | Easing/timing/iteration corpus | NaN/out-of-range/overflow/phantom-frame and ambiguous-completion corpus | Normalized progress/completion records | M9B | blocked | Required |
| M9MOTION-05 | Animatability is explicit per property: compatible color/brush/opacity/presentation-transform/radius/shadow/logical numeric families interpolate continuously, while resource/path/incompatible-gradient/auto-intrinsic/typography identity changes follow explicit discrete rules rather than generic Rust arithmetic. | Property compatibility/interpolation corpus | Generic-Lerp/path-morph/resource-crossfade/text-scale inference audit | Property interpolation decision records | M9B | blocked | Required |
...
| M9MOTION-07 | Reduced-motion behavior is derived only from explicit `StylePreferences::reduced_motion` plus bounded RunenUI policy: transitions/finite decorative timelines default to snap-to-end, repeating decorative timelines default to hold-initial, and preserved essential motion requires explicit declaration. | Preference/motion-policy corpus | Ambient platform/renderer policy, hidden speed multiplier, and ignored-preference audit | Reduced-motion decision/cancellation records | M9B | blocked | Required |
...
| M9MOTION-10 | Animation inspection extends the canonical runtime diagnostics/trace with target/start/replacement/cancellation/completion/sample/reduced-motion facts without creating a second mutable animation log or requiring application actions to implement debug/tween traits. | Trace/inspection corpus | Parallel event-log/debug-trait/action-channel audit | Canonical motion trace records | M9B | blocked | Required |

## M9C — integrated production closure

| ID | Required observation | Positive proof owner | Negative proof owner | Diagnostic / trace proof owner | Delivery slice | Status | Gate |
|---|---|---|---|---|---|---|---|
| M9INTEG-01 | One runtime-sampled node presentation transform is composed consistently into visible paint, physical hit testing, directional-focus geometry, semantic bounds, and presentation-relative clips; singular transforms have the inherited empty-hit behavior rather than stale layout fallback. | Transform/paint-hit-focus-semantic correlation corpus | Paint-only transform/stale hit/accessibility/untransformed fallback audit | Node/sample/geometry correlation records | M9C | blocked | Required |
| M9INTEG-02 | Deterministic public/headless proof advances `ManualClock` explicitly and observes exact intermediate/end/replacement/reduced-motion/layout/text/semantic results through ordinary public runtime contracts without sleeps, private expected runtime, alternate scene engine, or software expected renderer. | Public manual-time integration corpus | Sleep/wall-clock/private-model/alternate-engine audit | Fixture/time/publication diagnostics | M9C | blocked | Required |
