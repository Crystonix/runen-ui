...
RunenUI provides a deterministic headless framework foundation with the complete accepted M6 renderer-neutral paint/hit protocol, the complete accepted M7 reference production spine at proof maturity, and the complete accepted M8 production style/layout/international-text foundation. M8A provides deterministic production style resolution and invalidation; M8B provides explicit deterministic/production font-source policy, international shaping and line breaking behind RunenUI-owned contracts, immutable logical artifacts shared by measurement and paint, retained scale-independent shaped-resource bindings, and renderer-owned per-glyph SDF/MSDF realization without shaped-text provider or alpha fallback; M8C uses private low-level Taffy Block/Flex/Grid algorithms over the mounted topology with normalized sizing/positioning/overflow semantics, bounded downstream custom measurement, and final-layout-correlated text state while preserving one runtime-owned final geometry authority; M8D proves those mechanisms as one responsive/text-heavy production path through exact measurement/reflow records, retained artifact/resource identity, paint, semantic text/bounds, retained retry/raster-scale re-realization, deterministic bundled-font public tests, and real-wgpu multiscript evidence. See [status](../status.md) for accepted maturity and the [roadmap](../roadmap.md) for durable sequencing.
